...
public interface ReminderRepository extends JpaRepository<Reminder, Long> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT r FROM Reminder r JOIN FETCH r.policy p WHERE r.lastReminderSentAt BETWEEN :start AND :end AND p.status != 'PENDING_ISSUANCE' AND (r.lastCallOutcome IS NULL OR LOWER(r.lastCallOutcome) NOT IN ('pending issuance', 'renewed', 'externally renewed')) AND (:branch IS NULL OR :branch = '' OR p.branch = :branch)")
    List<Reminder> findByLastReminderSentAtBetweenWithValidPolicy(
        @org.springframework.data.repository.query.Param("start") java.time.LocalDateTime start, 
        @org.springframework.data.repository.query.Param("end") java.time.LocalDateTime end,
        @org.springframework.data.repository.query.Param("branch") String branch);

    @org.springframework.data.jpa.repository.Query("SELECT r FROM Reminder r JOIN FETCH r.policy p WHERE r.followUpDate BETWEEN :start AND :end AND p.status != 'PENDING_ISSUANCE' AND (r.lastCallOutcome IS NULL OR LOWER(r.lastCallOutcome) NOT IN ('pending issuance', 'renewed', 'externally renewed')) AND (:branch IS NULL OR :branch = '' OR p.branch = :branch)")
    ...
    @org.springframework.data.jpa.repository.Query("SELECT r FROM Reminder r JOIN FETCH r.policy p WHERE FUNCTION('DATE', r.followUpDate) IN :targetDates AND p.status != 'PENDING_ISSUANCE' AND (r.lastCallOutcome IS NULL OR LOWER(r.lastCallOutcome) NOT IN ('pending issuance', 'renewed', 'externally renewed'))")
    ...
    @org.springframework.data.jpa.repository.Query("SELECT r FROM Reminder r JOIN FETCH r.policy p WHERE r.followUpDate < :endOfTargetDate AND p.status != 'PENDING_ISSUANCE' AND (r.lastCallOutcome IS NULL OR LOWER(r.lastCallOutcome) NOT IN ('pending issuance', 'renewed', 'externally renewed')) AND (:branch IS NULL OR :branch = '' OR p.branch = :branch)")
